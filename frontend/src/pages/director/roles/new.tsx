...
import { Shield, ArrowLeft, Loader2, GripVertical, X, Check, Info, Key, Users, Building2, Package, Calendar, MapPin, DollarSign, Activity, BarChart3, Wrench, Truck } from 'lucide-react';
...
const PERMISSION_DESCRIPTIONS: Record<string, string> = {
    ...
    SECTION_DISEASE_CREATE: 'Kasallik qaydini yaratish',
    SECTION_DISEASE_UPDATE: 'Kasallik qaydini tahrirlash',
    SECTION_DISEASE_VIEW: 'Kasallik qaydlarini ko\'rish',
    ...
    INVENTORY_APPROVE: 'Inventarni tasdiqlash',
    INVENTORY_ALERT_VIEW: 'Inventar ogohlantirishlarini ko\'rish',
    INVENTORY_ALERT_RESOLVE: 'Inventar ogohlantirishlarini hal qilish',
    MEDICATION_READ: 'Dorilarni ko\'rish',
    MEDICATION_CREATE: 'Dori qo\'shish',
    MEDICATION_UPDATE: 'Dori tahrirlash',
    MEDICATION_DELETE: 'Dori o\'chirish',
    DISEASE_READ: 'Kasalliklarni ko\'rish',
    DISEASE_CREATE: 'Kasallik qo\'shish',
    DISEASE_UPDATE: 'Kasallik tahrirlash',
    DISEASE_DELETE: 'Kasallik o\'chirish',
    ATTENDANCE_READ: 'Yo\'qlamani ko\'rish',
    ATTENDANCE_CREATE: 'Yo\'qlama belgilash',
    ATTENDANCE_UPDATE: 'Yo\'qlamani tahrirlash',
    ATTENDANCE_DELETE: 'Yo\'qlamani o\'chirish',
    ATTENDANCE_APPROVE: 'Yo\'qlamani tasdiqlash',
    GPS_MONITOR_READ: 'GPS monitoringni ko\'rish',
    GPS_MONITOR_UPDATE: 'GPS sozlamalarini tahrirlash',
    REPORT_VIEW: 'Hisobotlarni ko\'rish',
    REPORT_EXPORT: 'Hisobotlarni eksport qilish',
    DASHBOARD_READ: 'Dashboardni ko\'rish',
    KPI_READ: 'KPI ko\'rsatkichlarini ko\'rish',
    ...
    FINANCE_EXPENSE_APPROVE: 'Xarajatni tasdiqlash',
    DELEGATE_PERMISSIONS: 'Ruxsatlarni vaqtinchalik o\'tkazish',
    ASSET_MANAGE: 'Aktivlarni boshqarish',
    TECH_REPORT_CREATE: 'Texnik hisobot yaratish',
    TECH_REPORT_VIEW: 'Texnik hisobotlarni ko\'rish',
    TECH_REPORT_UPDATE: 'Texnik hisobotni tahrirlash',
    FEED_MANAGE: 'Yem boshqarish',
    WATER_REPORT: 'Suv hisoboti',
    ELECTRICITY_REPORT: 'Elektr hisoboti',
    GAS_REPORT: 'Gaz hisoboti',
...
const PERMISSION_GROUPS = [
    ...
    },
    {
        ...
        permissions: ['SECTION_CREATE', 'SECTION_UPDATE', 'SECTION_VIEW', 'SECTION_ASSIGN_WORKER', 'SECTION_CLOSE', 'SECTION_STATUS_UPDATE'],
    },
    {
        name: 'Kunlik Hisobot',
        icon: Activity,
        color: 'text-cyan-500 bg-cyan-50 dark:bg-cyan-900/20',
        permissions: ['SECTION_DAILY_REPORT_CREATE', 'SECTION_DAILY_REPORT_UPDATE', 'SECTION_DAILY_REPORT_VIEW'],
    },
    {
        name: 'Partiya',
        icon: Package,
        color: 'text-orange-500 bg-orange-50 dark:bg-orange-900/20',
        permissions: ['BATCH_CREATE', 'BATCH_CLOSE', 'CHICK_OUT_CREATE', 'CHICKOUT_COMPLETE'],
    },
    {
        name: 'Davr',
        icon: Calendar,
        color: 'text-indigo-500 bg-indigo-50 dark:bg-indigo-900/20',
        permissions: ['PERIOD_CREATE', 'PERIOD_VIEW', 'PERIOD_UPDATE', 'PERIOD_CLOSE', 'PERIOD_EXPENSE_CREATE'],
    },
    {
        name: 'Ombor',
        icon: Package,
        color: 'text-amber-500 bg-amber-50 dark:bg-amber-900/20',
        permissions: ['WAREHOUSE_VIEW', 'WAREHOUSE_IN', 'WAREHOUSE_OUT', 'WAREHOUSE_UPDATE'],
    },
    {
        name: 'Inventar',
        icon: Package,
        color: 'text-teal-500 bg-teal-50 dark:bg-teal-900/20',
        permissions: ['INVENTORY_CREATE', 'INVENTORY_READ', 'INVENTORY_UPDATE', 'INVENTORY_DELETE', 'INVENTORY_APPROVE', 'INVENTORY_ALERT_VIEW', 'INVENTORY_ALERT_RESOLVE'],
    },
    {
        name: "Yo'qlama",
        icon: MapPin,
        color: 'text-green-500 bg-green-50 dark:bg-green-900/20',
        permissions: ['ATTENDANCE_READ', 'ATTENDANCE_CREATE', 'ATTENDANCE_UPDATE', 'ATTENDANCE_DELETE', 'ATTENDANCE_APPROVE', 'GPS_MONITOR_READ', 'GPS_MONITOR_UPDATE'],
    },
    {
        name: 'Hisobotlar',
        icon: BarChart3,
        color: 'text-violet-500 bg-violet-50 dark:bg-violet-900/20',
        permissions: ['REPORT_VIEW', 'REPORT_EXPORT', 'DASHBOARD_READ', 'KPI_READ'],
    },
    {
        name: 'Ish Haqi',
        icon: DollarSign,
        color: 'text-lime-500 bg-lime-50 dark:bg-lime-900/20',
        permissions: ['SALARY_VIEW', 'SALARY_MANAGE', 'SALARY_ADVANCE_GIVE', 'SALARY_BONUS_GIVE'],
    },
    {
        name: 'Moliya',
        icon: DollarSign,
    ...
    },
    {
        ...
        permissions: ['ASSET_MANAGE', 'TECH_REPORT_CREATE', 'TECH_REPORT_VIEW', 'TECH_REPORT_UPDATE'],
    },
    {
        name: 'Yem & Kommunal',
        icon: Truck,
        color: 'text-orange-600 bg-orange-50 dark:bg-orange-900/20',
        permissions: ['FEED_MANAGE', 'WATER_REPORT', 'ELECTRICITY_REPORT', 'GAS_REPORT', 'PRICE_MANAGE'],
    },
];
...
function PermissionItem({
    ...
    perm: string;
    isSelected: boolean;
    onDragStart: (e: React.DragEvent, perm: string) => void;
    ...
    return (
        <div
            className="relative"
            onMouseEnter={() => isSelected ? setShowRemove(true) : setShowTooltip(true)}
            onMouseLeave={() => { setShowTooltip(false); setShowRemove(false); }}
        >
            <div
                draggable={!isSelected}
                onDragStart={(e) => onDragStart(e, perm)}
                className={`
                    flex items-center gap-1.5 px-2 py-1 rounded text-xs font-medium transition-all cursor-grab
                    ${isSelected
                        ? 'bg-slate-200 dark:bg-slate-700 text-slate-400 cursor-not-allowed'
                        : 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 active:cursor-grabbing'
                    }
                `}
            >
            ...
            <AnimatePresence>
                {showTooltip && !isSelected && PERMISSION_DESCRIPTIONS[perm] && (
                    <motion.div
                        ...
                        exit={{ opacity: 0, y: 5 }}
                        className="absolute z-50 left-0 top-full mt-1 px-2 py-1 bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 text-xs rounded shadow-lg whitespace-nowrap max-w-xs"
                    >
            ...
            <AnimatePresence>
                {showRemove && isSelected && (
                    <motion.button
                        ...
                        className="absolute -top-1 -right-1 h-4 w-4 bg-red-500 text-white rounded-full flex items-center justify-center shadow"
                    >
...
function SelectedPermissionBadge({ perm, onRemove }: { perm: string; onRemove: (perm: string) => void }) {
    ...
    return (
        <motion.div
            ...
            className="relative inline-flex items-center gap-1 px-2 py-1 bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300 rounded text-xs font-medium"
        >
...
export default function NewRolePage() {
    ...
    const handleDragStart = (e: React.DragEvent, perm: string) => {
        e.dataTransfer.setData('permission', perm);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (form.permissions.length === 0) {
            setError("Kamida bitta permission tanlang (chapdan sudrab tashlang)");
            return;
        }
                ...
                <div className="sticky top-4 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden">
                    ...
                    <div className="h-[calc(100vh-200px)] overflow-y-auto p-3 space-y-4">
                        {PERMISSION_GROUPS.map(group => {
                            ...
                            return (
                                <div key={group.name}>
                                    <div className={`flex items-center gap-2 px-2 py-1 rounded ${group.color}`}>
                                    ...
                                    <div className="flex flex-wrap gap-1.5 mt-2">
                                        {group.permissions.map(perm => (
                                            <PermissionItem
                                                key={perm}
                                                perm={perm}
                                                isSelected={form.permissions.includes(perm)}
            ...
            <motion.div variants={itemVariants} className="flex-1 max-w-xl">
                <div className="flex items-center gap-4 mb-6">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Yangi rol yaratish</p>
                    </div>
                </div>
                    ...
                    <div className="space-y-5">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                placeholder="Masalan: Ishchi, Menejer, Hisobchi"
                            />
                        </div>
                        ...
                        <div>
                            ...
                            <input
                                ...
                                min="0"
                            />
                            <p className="text-xs text-slate-400 mt-1">Bu lavozimga tayinlangan ishchilarning default oyligi</p>
                        </div>

                        {/* Drop Zone */}
                        <div>
                            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
                                Tanlangan ruxsatlar ({form.permissions.length} ta)
                            </label>
                            <div
                                ref={dropZoneRef}
                                ...
                                className={`
                                    ...
                                    ${form.permissions.length === 0 ? 'flex items-center justify-center' : ''}
                                `}
                            >
                                {form.permissions.length === 0 ? (
                                    <p className="text-sm text-slate-400 text-center">
                                        {isDragOver ? '🎯 Shu yerga tashlang!' : '📋 Chapdan ruxsatlarni shu yerga sudrab tashlang'}
                                    </p>
                                ) : (
                                    <div className="flex flex-wrap gap-2">
                                        <AnimatePresence>
                                            {form.permissions.map(perm => (
                                                <SelectedPermissionBadge
                                                    key={perm}
                                                    perm={perm}
                                                    onRemove={removePermission}
                                                />
